# 낙하하는 미사일을 요격하는 게임 마우스 커서로 조준해 요격 미사일을 발사
import sys
...
class House:
    ...
    def __init__(self, xpos):
        self.rect = Rect(xpos, 550, 40, 40)  # 집 크기 위치
        self.exploded = False  # 폭발했는지 아닌지
        ...
        self.images[1].blit(strip, (0, 0), Rect(20, 0, 20, 20))  # 집이미지와 폭발시 이미지
...
class Missile:
    """ 낙하하는 미사일 객체 """
    def __init__(self):
        self.max_count = 500  # 발사부터 지면에 도착하기까지 필요한 시간
        self.interval = 1000  # 낙하와 낙하의 간격
        self.pos = [0, 0]  # 낙하 좌표(낙하 시작점, 도착점)
        self.cpos = [0, 0]  # 현재 낙하 중인 좌표의 x좌표 y좌표
        self.firetime = 0  # 낙하 시작 시간
        self.radius = 0  # 폭풍 반경
        self.reload(0)  # 미사일을 초기화 한다
