...
import pygame
from pygame.locals import QUIT, MOUSEBUTTONDOWN, BUTTON_LEFT
...
def num__of_bomb(field, x_pos, y_pos):
    """ 주위에 있는 폭탄 수를 반환한다 """
    count = 0
...
def open_tile(field, x_pos, y_pos):
    """ 타일을 오픈 """
    global OPEN_COUNT  # 전역변수를 수정하기 위해 사용한다 명시
    if CHECKED[y_pos][x_pos]: return  # 이미 확인된 타일
    ...
    for yoffset in range(-1, 2):
        for xoffset in range(-1, 2):
            ...
            if 0 <= xpos < WIDTH and 0 <= ypos < HEIGHT and \
                ...
                OPEN_COUNT += 1
                count = num__of_bomb(field, xpos, ypos)
                if count == 0 and \
