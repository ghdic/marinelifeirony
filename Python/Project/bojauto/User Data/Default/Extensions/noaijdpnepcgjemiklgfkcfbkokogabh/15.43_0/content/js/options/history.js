 ...
 cleanText = GEBI(ob.id).innerHTML.replace(/<\/?[^>]+(>|$)/g, "");
	   ...
	   switch(localStorage["SL_SLVoices"]){
		case "0": if(ALLvoices.indexOf(code)!=-1){
				...
				if(text.length>GTTS_length) window.open("http://text-to-speech.imtranslator.net/?dir="+code+"&text="+encodeURIComponent(text)); 
		...
		case "2": if(ALLvoices.indexOf(code)!=-1){
                              if(SL_TTS.indexOf(code)!=-1) window.open("http://text-to-speech.imtranslator.net/?dir="+code+"&text="+encodeURIComponent(text));
			      else setTimeout(function(){REMOTE_Voice(code,text);}, 500);
			  } else alert(noVoice);
			  break;
	   }
