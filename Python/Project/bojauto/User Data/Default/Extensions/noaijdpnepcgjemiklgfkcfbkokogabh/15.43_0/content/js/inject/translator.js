		...
		if(TranslatorIM.SL_OnOff_HK=="true" && TranslatorIM.SL_ENABLE=="true"){
			...
			if(TranslatorIM.SL_SYNC_FILTER(DBHOTKEYSline1,HOTKEYSline)==true) TranslatorIM.SL_ShowBalloon();
			  ...
			  if(doc.getElementById("SL_balloon_obj")){
				...
				doc.body.removeChild (doc.getElementById("SL_balloon_obj"));
				TranslatorIM.SL_OBJ_BUILDER();
                                if(TranslatorIM.SL_DBL=="true") TranslatorIM.SL_ShowBalloon();


				if(TranslatorIM.SL_TMPbox=="true") doc.getElementById('SL_locer').checked=false;
				else doc.getElementById('SL_locer').checked=true;
...
*/
	...
	SL_ShowButton: function(){
	 ...
	 TranslatorIM.TIMEOUT = setTimeout(function() {
	   if(TranslatorIM.SL_BBL_TMP_BLOCK==1){
	        var doc = FExtension.browserInject.getDocument();
...
/*
		if(TranslatorIM.SL_FRAME==1){
			for(var i=0; i<top.window.frames.length; i++){		
			  if (top.window.frames[i].document.hasFocus()==false) {
                            if(top.window.frames[i].document.getElementById("SL_balloon_obj")){
				top.window.frames[i].document.body.removeChild (top.window.frames[i].document.getElementById("SL_balloon_obj"));
				var sel = top.window.frames[i].getSelection ? top.window.frames[i].getSelection() : top.window.frames[i].document.selection;
				sel.removeAllRanges();
			    }
			  }
			}
...
*/
		...
		var SL_P4 = doc.getElementById("SL_PN4");

		TranslatorIM.SL_TMPbox = TranslatorIM.SL_NODETECT_bbl;

		if(TranslatorIM.SL_TS==TranslatorIM.SL_TSold){
			if(doc.getElementById('SL_locer').checked==false) TranslatorIM.SL_TMPbox = "false";
		...
		if(SLdivField.style.backgroundColor==""){
			...
			if(theSLtext != ""){
				...
				TranslatorIM.SL_addEvent(SLdivField, 'mouseup', TranslatorIM.SL_ShowBalloon);
			    	...
			    	TranslatorIM.SL_addEvent(SL_P4, 'click', TranslatorIM.SL_retranslate);  
	...
	SL_BBL_VOICE: function(){
	   ...
	   switch(TranslatorIM.SL_SLVoices){
		case "0": if(TranslatorIM.SL_ALLvoices.indexOf(SL_to)!=-1){
				...
				if(text.length>TranslatorIM.GTTS_length) window.open("http://text-to-speech.imtranslator.net/?dir="+SL_to+"&text="+encodeURIComponent(text)); 
	...
	SL_BBL_locer_reset: function(){
       		if(TranslatorIM.SL_OnOff_BTN=="false") TranslatorIM.SL_BBL_TMP_BLOCK=0;
       		else TranslatorIM.SL_BBL_TMP_BLOCK=1;		
	},

	SL_BBL_locer_settler: function(){
          var doc = FExtension.browserInject.getDocument();
          if(TranslatorIM.SL_TS!=TranslatorIM.SL_TSold){		
                TranslatorIM.SL_BBL_locer_reset();
				doc.body.removeChild (doc.getElementById("SL_balloon_obj"));
				TranslatorIM.SL_OBJ_BUILDER();

		if(TranslatorIM.SL_BBL_TMP_BLOCK==1){
			doc.getElementById('SL_BBL_locer').checked=true;
		}else{
			doc.getElementById('SL_BBL_locer').checked=false;
		}
	   }else{
		if(TranslatorIM.SL_OnOff_BTN=="false"){
			TranslatorIM.SL_BBL_TMP_BLOCK=0;
		} else {
			TranslatorIM.SL_BBL_TMP_BLOCK=1;
		}
	   }
	},
	...
	SL_BBL_locer: function(){
			...
			TranslatorIM.SL_BBL_locer_reset(); 
				...
				if(doc.getElementById('SL_BBL_locer').checked==false){
					TranslatorIM.SL_BBL_TMP_BLOCK=0;
					TranslatorIM.SL_OnOff_BTN="false";
				} else {
					TranslatorIM.SL_BBL_TMP_BLOCK=1;
					TranslatorIM.SL_OnOff_BTN="true";
				}
			}
		}else{
			if(TranslatorIM.SL_OnOff_BTN=="false"){
	...
	SL_locker: function(){	
	        var doc = FExtension.browserInject.getDocument();
		if(doc.getElementById('SL_locer').checked==false) TranslatorIM.SL_TMPbox=="true";
...
//alert(TranslatorIM.SL_langSrc + " / " + TranslatorIM.SL_langDst)
	   if(TranslatorIM.SL_TS!=TranslatorIM.SL_TSold){
		doc.getElementById("SL_lng_to").value=TranslatorIM.SL_langDst;
		doc.getElementById("SL_lng_from").value=TranslatorIM.SL_langSrc;
	   }
	...
	SL_Voice: function(){
	   ...
	   switch(TranslatorIM.SL_SLVoices){
		case "0": if(TranslatorIM.SL_ALLvoices.indexOf(SL_from)!=-1){
				...
				if(text.length>TranslatorIM.GTTS_length) window.open("http://text-to-speech.imtranslator.net/?dir="+SL_from+"&text="+encodeURIComponent(text)); 
...
//	  if(BackUpDir=="es") client="t";
	...
	SL_CopyToClipBoard: function(){
		...
		//OBJ.innerHTML = ZIPtext.replace(/(<([^>]+)>)/ig,'').replace(/&nbsp;/ig,'');
...
//		myTransText = myTransText.replace(/#/g,"");
	...
	//			if(Stemp=="en" || TranslatorIM.SL_FLAG==0) Stemp="auto";
				...
				if(theQ==1){
			        ...
			        FExtension.browserInject.runtimeSendMessage({greeting: "TR_GOOGLE:>"+baseUrl+","+ SL_Params}, function(response) {});
				setTimeout(function(){
				    var SLIDL="";
				    clearInterval(SLIDL);
				    var SLIDL = setInterval(function(){
 ...
 return(OUT);
	...
	tagClick: function(event,id){
	   ...
	   switch(TranslatorIM.SL_SLVoices){
		...
		} else window.open("http://text-to-speech.imtranslator.net/?dir="+SL_to+"&text="+encodeURIComponent(text));
	},
...
//					        			OB23.setAttributeNode(js);
		...
		OBtbl.height = "16";
				...
				OBtd3_.align = "center";
					...
					va.value = TranslatorIM.SL_BBL_TMP_BLOCK;
				...
				OBtd2.align = "center";
					...
					hr = doc.createAttribute("href");
					hr.value ="http://imtranslator.net"+_CGI+"&a=0";
		...
		if(doc.getElementById('SL_lng_from')){
			if(TranslatorIM.SL_TS==TranslatorIM.SL_TSold && TranslatorIM.SL_SID_FROM!=""){
				...
				if(TranslatorIM.SL_TMPbox=="true") doc.getElementById('SL_locer').checked=false;
	        	        ...
	        	        doc.getElementById('SL_lng_from').value=TranslatorIM.SL_langSrc;
	                	doc.getElementById('SL_lng_to').value=TranslatorIM.SL_langDst;
				if(CHbox=="true") doc.getElementById('SL_locer').checked=false;
				else doc.getElementById('SL_locer').checked=true;
			}
		}
	...
	fade: function (){
	 ...
	 setTimeout(function() { 
		...
		if(THEobj){
			 ...
			 THEobj.style.transition='visibility 1s, opacity 1s linear';
		}
	 ...
	 SL_WPT_TABS.setAttributeNode(id);
	  ...
	  SL_WPT_TAB2div.onclick = function(){
		...
		SL_WPT_TAB6div.onclick = function(){
		    window.open("http://imtranslator.net"+_CGI+"&a=0","ImTranslator: Buy a coffee");
		};
		var id = doc.createAttribute("id");
...
//			 	case "en":LNG = "en-GB|Male"; break;
	...
	SL_YANDEX: function(text,f,t){
		...
		if(f==t){
			doc.getElementById('SL_shadow_translator').style.display = 'block';

 	                resp=text;

			TranslatorIM.SL_temp_result=resp;

	                var resptmp = TranslatorIM.PPB_tts_icon(t,resp);
