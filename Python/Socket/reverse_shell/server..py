...
def download(filee):
    cmd = filee
    filee = "".join(file.split(":download")).strip()
    if file.strip():
        filetodown = file.split("/")[-1] if "/" in filee else filee.split("\\")[-1] if "\\" in filee else filee
        controler.send(cmd.encode('UTF-8'))
        down = controler.recv().decode("UTF-8", "ignore")
        if down == "true":
            print(f"[~] Downloading [ {filetodown} ]...")
...
def server(IP, PORT, senrev=senrev):
    ...
    s.setsockopt(socket.SOL_SOCKET, socekt.SO_RESUEADDR, 1)
    s.bind((IP, PORT))
    s.listen(1)
