...
class KakaoTalkChatBot:
    ...
    def kakao_sendtext(self, text, chatroom_name):
        """ 메세지 보내기 """
        # HWND FindWindowEx(
        ...
        hwndMain = win32gui.FindWindow(None, chatroom_name)  # 해당하는 이름의 채팅 창 찾음
    ...
    def check_open(self, chatroom_name):
        """ 채팅방이 열려있는지 확인, 열려있는경우 핸들번호, 닫혀있는 경우 0 반환 """
        hwndchatroom = win32gui.FindWindow(None, chatroom_name)
        return hwndchatroom

    def close_window(self, chatroom_name):
        """ 윈도우를 닫는다 """
        hwndchatroom = win32gui.FindWindow(None, chatroom_name)
        win32gui.CloseWindow(hwndchatroom)
        time.sleep(self.room_close_delay)

    def refresh_window(self, chatroom_name):
        """ 쌓이는 메세지를 한번 초기화 해준다(너무 많이 쌓이면 랙걸림) """
        self.close_window(chatroom_name)
        self.open_chatroom(chatroom_name)


    def open_chatroom(self, chatroom_name, kind="채팅방"):
        ...
        cnt = 0
        while not self.check_open(chatroom_name):  # 열릴때까지 반복
    ...
    def get_chatroom_msg(self, chatroom_name):
        hwndMain = win32gui.FindWindow(None, chatroom_name)
        hwndListControl = win32gui.FindWindowEx(hwndMain, None, 'EVA_VH_ListControl_Dblclk', None)

        self.PostKeyEx(hwndListControl, ord('A'), [self.w.VK_CONTROL], False)
        time.sleep(self.copy_message_delay)
        self.PostKeyEx(hwndListControl, ord('C'), [self.w.VK_CONTROL], False)
